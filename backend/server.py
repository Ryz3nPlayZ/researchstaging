...
from literature_v2_api import router as literature_v2_router

logger.info(
    f"Loaded literature V2 router with {len(literature_v2_router.routes)} routes"
)
    ...
    try:
        if data.code:
            ...
            return LoginResponse(
                user=UserResponse(**result["user"]), token=result["token"]
            )
        elif (
            data.email and os.environ.get("ENVIRONMENT", "development") != "production"
        ):
            # Mock auth — only allowed in development
            result = await auth_service.mock_authenticate_user(
                data.email, data.name, db
            )
            return LoginResponse(
                user=UserResponse(**result["user"]), token=result["token"]
            )
...
async def create_project(
    ...
    try:
            ...
            phases=[
                {
                    ...
                    "tasks": [
                        ...
                        },
                        {
                            ...
                            "dependencies": ["Literature Search"],
                        },
                    ],
                },
                {
                    ...
                    "tasks": [
                        {
                            ...
                            "type": "drafting",
                            "description": "Draft final document",
        ...
        # Expand plan into tasks
        logger.info(f"Expanding plan to tasks for project {project.id}...")
        tasks = await orchestration_engine.expand_plan_to_tasks(
            db, project.id, default_plan
        )
        logger.info(f"Created {len(tasks)} tasks for project {project.id}")
...
async def list_projects(
    db: AsyncSession = Depends(get_db), current_user: DBUser = Depends(require_auth)
):
...
async def execute_project(
    ...
    await connection_manager.publish_event(
        project_id,
        "execution_started",
        {"project_id": project_id, "tasks_queued": len(ready_tasks)},
    )
...
async def add_paper_to_project(
    ...
    # Create paper
    paper = Paper(project_id=project_id, **paper_create.model_dump())
    db.add(paper)
    await db.commit()
...
@api_router.get(
    "/projects/{project_id}/execution-logs", response_model=List[ExecutionLogResponse]
)
...
async def get_agent_graph(
    ...
    result = await db.execute(
        select(Task.task_type, Task.state).where(Task.project_id == project_id)
    )
    ...
    agents = [
        # Orchestrator (top level)
        {
            ...
            "name": "Orchestrator",
            "type": "orchestrator",
            "description": "Main session coordination and planning",
        },
        # Executor agents
        {
            "id": "executor",
            ...
            "type": "executor",
            "description": "Execute PLAN.md files atomically",
        },
        # Verifier agents
        {
            "id": "verifier",
            "name": "Verifier Agents",
            "type": "verifier",
            "description": "Verify phase goals against codebase",
        },
        ...
        },
        {
            ...
            "type": "specialist",
            "description": "Research unknown domains before planning",
        },
        {
            "id": "debugger",
            "name": "System Debugger",
            "type": "specialist",
            "description": "Investigate bugs systematically",
        },
    ]
    ...
    for agent in agents:
        ...
        if agent["id"] == "executor":
            ...
            agent_status = (
                "running"
                if any(s == TaskState.RUNNING for s in task_states.values())
                else "idle"
            )
...
@api_router.get(
    "/projects/{project_id}/artifacts", response_model=List[ArtifactResponse]
)
...
async def export_artifact(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{artifact.title}.{extensions.get(request.format, "txt")}"'
        },
    )
...
async def list_project_papers(
    ...
    if search:
        ...
        query = query.where(
            (Paper.title.ilike(search_pattern)) | (Paper.abstract.ilike(search_pattern))
        )
            ...
            nodes.append(
                {
                    ...
                    "data": {
                        "label": ref.title[:40] + "..."
                        if ref.title and len(ref.title) > 40
                        else (ref.title or "Unknown"),
...
async def perform_ai_action(
    request: AIActionRequest, _user: DBUser = Depends(require_auth)
):
...
async def get_global_stats(
    db: AsyncSession = Depends(get_db), _user: DBUser = Depends(require_auth)
):
...
api_router.include_router(literature_v2_router)
logger.info("Included literature V2 router in api_router")
...
if not _cors_origins:
    logger.warning(
        "CORS_ORIGINS not set — defaulting to localhost:3000. Set explicitly for production."
    )
